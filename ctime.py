...
# ------------------TAC------------------------------------------------------
#
# TAC performs temporal arc consistency. It maintains a set of Cintervals and Cinstants
# to check consistency.  If non are provided, it retrieves all Cintervals and Cinstants
#
#	g: graph
#	todo_cint: list of Cintervals to be TAC'd
#	todo_cins: list of Cinstants to be TAC'd

def TAC(g, todo_cint = set(), todo_cins = set()) :
...
def TAC_intervalBefore(g1, cDTI1, g2, cDTI2) :
	...
	p2bmin = g1.value(p2bmin_i, time.inDateTime, any=False)
...
def add_CDTInterval(g, int, bmin, bmax, emin, emax) :
	...
	create_CDTInstant(g,b, bmin, bmax)

	e = BNode()
	g.add((int, time.hasEnd, e))
	g.add((e, RDF.type, cot.CDateTimeInstant))
	create_CDTInstant(g,e, emin, emax)
